...
export class ProductValidationCollector {
  ...
  public async validateAndFilterProducts(
    ...
    this.updateValidationProgress({
      ...
      percentage: 0,
      currentStep: "1.5/3단계: DB 중복 검증 중",
      message: `1.5/3단계: ${products.length}개 제품 유효성 검증 중`
    });
  ...
  private updateValidationProgress(data: Partial<CrawlingProgress>): void {
    ...
    this.state.updateProgress({
      ...currentProgressData,
      ...data,
      stage: data.currentStep?.includes('완료') ? 'validation:complete' : 'validation:processing'
    });
  }
}
