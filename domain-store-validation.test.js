...
describe('Domain Store Pattern Validation', () => {
  ...
  it('should handle progress updates correctly', () => {
    const testProgress = {
      ...crawlingStore.progress.get(),
      percentage: 50
    };
    crawlingStore.progress.set(testProgress);
    expect(crawlingStore.progress.get().percentage).toBe(50);
    
    testProgress.percentage = 100;
    crawlingStore.progress.set(testProgress);
    expect(crawlingStore.progress.get().percentage).toBe(100);
  });
